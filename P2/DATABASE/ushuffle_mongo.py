...
from ushuffle_dbU import DBNAME,randName,FIELDS,tformatt,cformat
...
class MongoTest(object):
    ...
    def insert(self):
        self.users.insert(dict(login=who,userid=uid,projid=rand(1,5)) \
        for who,uid in randName())
    def update(self):
        fr=rand(1,5)
        to=rand(1,5)
        i=-1
        for i,user in enumerate(sele.users.find({'projid':fr})):
